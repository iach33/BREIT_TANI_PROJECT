...
# Read datasets
df_desnutricion = pd.read_excel("../Data/raw/DATA PROYECTO BREIT.xlsx", 
                                 sheet_name="DESNUTRICION")
df_desarrollo = pd.read_excel("../Data/raw/DATA PROYECTO BREIT.xlsx", 
                               sheet_name="DESARROLLO")
df_diccionario = pd.read_excel("../Data/raw/DATA PROYECTO BREIT.xlsx", 
                                sheet_name="DICCIONARIO DE DATOS")
```
...
profile_desnutricion = get_dataframe_profile(df_desnutricion, "DESNUTRICION")
profile_desarrollo = get_dataframe_profile(df_desarrollo, "DESARROLLO")
```
...
def parse_age_to_months(age_str):
    """
    Convert mixed age format to numeric months
    Examples: '6m' -> 6.0, '1a8m' -> 20.0, '4d' -> 0.13
    """
    if pd.isna(age_str):
        return np.nan
    ...
    # Extract years
    if 'a' in age_str:
        years = age_str.split('a')[0]
        if years.isdigit():
            months += int(years) * 12
        age_str = age_str.split('a')[1] if 'a' in age_str else ""
    
    # Extract months
    if 'm' in age_str:
        month_str = age_str.split('m')[0]
        if month_str.isdigit():
            months += int(month_str)
    
    # Extract days
    if 'd' in age_str:
        days = age_str.split('d')[0]
        if days.isdigit():
            months += int(days) / 30.0  # Approximate
...
def clean_numeric_field(series):
    """
    Convert text-based numeric fields to float
...
def standardize_categorical(series, mapping_dict):
    ...
    """
    return series.str.strip().str.upper().map(mapping_dict)
```
...
df_clean = pd.read_csv("../data/intermediate/tani_preprocessed_final_v2.csv")

# Clean age
#df_clean['edad_meses'] = df_clean['Edad'].apply(parse_age_to_months)

# Clean anthropometric measurements
df_clean['Peso_Num'] = clean_numeric_field(df_clean['Peso'])
df_clean['Talla_Num'] = clean_numeric_field(df_clean['Talla'])
df_clean['PC_Num'] = clean_numeric_field(df_clean['CabPC'])

# Clean hemoglobin
if 'Tam_hb' in df_clean.columns:
    df_clean['Hemoglobina'] = clean_numeric_field(df_clean['Tam_hb'])

# Display cleaning results
print("\nData Cleaning Results:")
print(f"Original records: {len(df_desnutricion):,}")
print(f"After cleaning and consolidation (pre and post pandemic datasets): {len(df_clean):,}")
print(f"\nAge conversion success rate: {df_clean['edad_meses'].notna().sum() / len(df_clean) * 100:.1f}%")
print(f"Weight conversion success rate: {df_clean['Peso_Num'].notna().sum() / len(df_clean) * 100:.1f}%")
print(f"Height conversion success rate: {df_clean['Talla_Num'].notna().sum() / len(df_clean) * 100:.1f}%")
```
...
print(df_clean[['edad_meses', 'Peso_Num', 'Talla_Num', 'PC_Num']].describe())

print("\nNutritional Status Distribution:")
for col in ['P/T', 'T/E', 'P/E']:
